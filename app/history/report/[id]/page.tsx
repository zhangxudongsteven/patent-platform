...
export default function ReportHistoryPage() {
  ...
  const loadHistory = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        if (historyRecord.operation_type === "report") {
          if (historyRecord.operation_content) {
            try {
              ...
              setFileName(historyRecord.operation_title || "专利检索报告");
            } catch (e) {
    ...
    return (
      <div className="flex flex-col h-full">
        <header className="flex items-center gap-2 border-b px-4 py-3">
          ...
          <span className="font-semibold">专利检索报告</span>
        </header>
  ...
  return (
    <div className="flex-1 flex-col">
      <ReportProvider initialStep={initialData?.step || 1} initialData={initialData} chatId={chatId}>
        <ReportWorkflow
          fileName={fileName}
